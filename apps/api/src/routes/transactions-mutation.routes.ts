...
import { z } from 'zod';
import { db } from '../db';
import { transactions, accounts, categorizationRules, categories, tags, transactionTags } from '../db/schema';
...
async function getOwnedTransaction(transactionId: string, userId: string) {
  const [txn] = await db
    .select({ id: transactions.id, description: transactions.description, accountId: transactions.accountId })
    .from(transactions)
...
router.post('/:id/tags', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const txn = await getOwnedTransaction(req.params.id, req.user!.id);
    if (!txn) {
      res.status(404).json({ error: 'Transaction not found' });
...
router.delete('/:id/tags/:tagId', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const txn = await getOwnedTransaction(req.params.id, req.user!.id);
    if (!txn) {
      res.status(404).json({ error: 'Transaction not found' });
