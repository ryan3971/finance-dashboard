...
const LINK_BASE = 'px-3 py-1.5 rounded text-sm transition-colors text-gray-500 hover:text-gray-900 hover:bg-gray-50';
const LINK_ACTIVE = 'px-3 py-1.5 rounded text-sm transition-colors bg-gray-100 text-gray-900 font-medium';
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  activeProps={{ className: 'block px-3 py-2.5 rounded text-sm bg-gray-100 text-gray-900 font-medium' }}
                  activeOptions={link.to === '/' ? { exact: true } : undefined}
                  onClick={() => setDrawerOpen(false)}
                >
                  {link.label}
